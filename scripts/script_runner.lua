...
function script_runner:menu()
	--if (CollapsingHeader("[Runner")) then
		...
		Separator()
		if (Button("Add location to DB")) then script_nav:newHotspot(GetMinimapZoneText() .. ' ' .. GetLocalPlayer():GetLevel() .. ' - ' .. GetLocalPlayer():GetLevel()+2); script_runner:printHotspot(); end
...
function script_runner:printHotspot()
	DEFAULT_CHAT_FRAME:AddMessage('script_runner: Add this hotspot to your database by adding the following line in the setup-function in script_runner.lua:');
	...
	local addString = 'script_runner:addDestination("' .. GetMinimapZoneText() .. '", ' .. hx .. ', ' .. hy .. ', ' .. hz .. ', 0, 0);'	
