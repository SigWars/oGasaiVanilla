...
-- SIG Check Actionbar spell if is Usable not need set slot number
function script_warrior:IsUsableAction(SpellName) 
	for z=1,172 do 
		local isUsable, _ = IsUsableAction(z);
		if (isUsable == 1) then 
			if (GetSpellName(z) == SpellName and not IsSpellOnCD(SpellName)) then
				return true;
			end
		end
	end
...
function script_warrior:run(targetGUID)
	...
	if (targetObj ~= 0) then
		...
		-- Combat
			...
			if (targetObj:GetDistance() < 0.3) then 
			...
			if (targetObj:GetDistance() <= self.meeleDistance) then
				...
				if (script_warrior:IsUsableAction('Overpower') and localRage >= 5 and not IsSpellOnCD('Overpower')) then 
				...
				if (targetObj:GetCreatureType() == 'Humanoid' and localRage >= 10 and targetHealth < 30 and not targetObj:HasDebuff('Hamstring')) then 
