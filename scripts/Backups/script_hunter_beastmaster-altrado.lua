...
function script_hunter:run(targetGUID)
	...
	if (script_follow.IgnoreAttacks) then
		return 4;
	end	
	-- Assign the target 
	...
	if (targetObj ~= 0 and targetObj ~= nil) then
		...
		-- Combat
			...
			if (targetObj:GetDistance() < 0.50 and not script_follow.IgnoreAttacks) then
			...
			if (targetObj:GetDistance() > 4 and targetObj:GetDistance() < 12 and not script_grind:isTargetingMe(targetObj) and targetObj:GetUnitsTarget() ~= 0) then
				if (hasPet and targetObj:GetUnitsTarget():GetGUID() == pet:GetGUID()) then
...
function script_hunter:doOpenerRoutine(targetGUID, pet) 
	...
	if (not IsAutoCasting('Auto Shot') and targetObj:GetDistance() < 35 and targetObj:GetDistance() > 8) then
		-- Dismount
	...
	if (not targetObj:IsInLineOfSight() or targetObj:GetDistance() > 35 or (targetObj:GetDistance() < 12 and targetObj:GetDistance() > 4) and not script_follow.IgnoreAttacks) then 
		return false;
	end 
...
function script_hunter:doInCombatRoutine(targetObj, localMana) 
	...
	if (targetObj:GetDistance() < 35 and targetObj:GetDistance() > 12) then
	...
	if (not targetObj:IsInLineOfSight() or targetObj:GetDistance() > 35) then
		return false;
	elseif (targetObj:GetDistance() < 8 and targetObj:GetDistance() > 5) then
		return false;
	else
	...
	if (not targetObj:IsInLineOfSight() or targetObj:GetDistance() > 35) then 
		...
		if (IsMoving() and targetObj:GetDistance() > 12) then 
			return true;
		end
	end
