script_priest_shadow = {
	message = 'Shadow - Priest Combat Script',
	drinkMana = 60,
	eatHealth = 80,
	isSetup = false,
	renewHP = 90,
...
function script_priest_shadow:healAndBuff(targetObject, localMana)
	...
	if (localMana > 20 and targetHealth < self.greaterHealHP) then
		if (script_priest_shadow:heal('Heal', targetObject)) then
			return true;
		end
	end
	...
	if (localMana > 10 and targetHealth < self.lesserHealHP) then
		if (script_priest_shadow:heal('Lesser Heal', targetObject)) then
			return true;
		end
	end
	...
	if (localMana > 8 and targetHealth < self.flashHealHP) then
		if (script_priest_shadow:heal('Flash Heal', targetObject)) then
			return true;
		end
	end
...
function script_priest_shadow:heal(spellName, target)
...
function script_priest_shadow:cast(spellName, target)
...
function script_priest_shadow:enemiesAttackingUs(range) -- returns number of enemies attacking us within range
...
function script_priest_shadow:runBackwards(targetObj, range) 
...
function script_priest_shadow:setup()
	self.waitTimer = GetTimeEX();
	self.isSetup = true;

end

function script_priest_shadow:draw()
	--script_priest_shadow:window();
...
function script_priest_shadow:run(targetGUID)
	
	if(not self.isSetup) then
		script_priest_shadow:setup();
	end
	...
	if (targetObj ~= 0 and targetObj ~= nil) then
		...
		if (not IsInCombat()) then
			...
			if (script_priest_shadow:cast('Devouring Plague', targetObj)) then
			...
			if (GetNumPartyMembers() == 0) then
				if (script_priest_shadow:cast('Mind Blast', targetObj)) then
					self.waitTimer = GetTimeEX() + 200;
					return 0;
				end
			end
			...
			if (not targetObj:HasDebuff("Shadow Word: Pain") and localMana > 75) then
				if (Cast('Shadow Word: Pain', targetObj)) then 
					return 0; 
				end
			end

			-- if (script_priest_shadow:cast('Smite', targetObj)) then
			--	self.waitTimer = GetTimeEX() + 200;
			--	return 0;
			-- end
			...
			-- Wand if low mana or target is low
			 if (self.useWand) then
				if (targetHealth >= 0 and localObj:HasRangedWeapon()) then
		...
		-- Combat COMBAT!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
			...
			if (script_priest_shadow:healAndBuff(localObj, localMana)) then
				return 0;
			end
			...
			if (script_priest_shadow:enemiesAttackingUs(5) > 1 and targetHealth > 20) then
			...
			if (not localObj:HasBuff('Inner Fire') and HasSpell('Inner Fire') and localMana > 75) then
			...
			if (localMana > 98 and Cast('Smite', targetObj)) then 
				return 0; 
			end
		end
	end
...
function script_priest_shadow:rest()
...
function script_priest_shadow:mount()
...
function script_priest_shadow:window()
...
function script_priest_shadow:menu()
